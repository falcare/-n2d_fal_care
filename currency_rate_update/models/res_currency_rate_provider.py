...
class ResCurrencyRateProvider(models.Model):
    ...
    service = fields.Selection(string="Source Service", selection=[("UAE", "Central Bank Of the UAE")], required=True)
    ...
    def _update(self, date_from, newest_only=False):
        Currency = self.env["res.currency"]
        ...
        for provider in self:
            try:
                ...
                provider.message_post(
                    ...
                    % (provider.name, date_from, str(e) if e else _("N/A"),),
                )
                continue
            ...
            for rates in data:
                cur = rates.get("cur")
                cur_rate = 1 / rates.get("rate")
                timestamp = fields.Date.today()
                currency = Currency.search([("name", "=", cur)], limit=1)
                if not currency:
                    raise UserError(
                        _("Unknown currency from %(provider)s: %(rate)s")
                        % {"provider": provider.name, "rate": rate}
                    )
                rate = provider._process_rate(currency, cur_rate)
    ...
    def _scheduled_update(self):
        ...
        if providers:
            ...
            for provider in providers.with_context({"scheduled": True}):
                ...
                provider._update(date_from, newest_only=True)
